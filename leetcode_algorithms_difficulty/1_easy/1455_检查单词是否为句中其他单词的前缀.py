...
class Solution:
    def isPrefixOfWord(self,sentence,searchWord):
        part = sentence.split(" ")      # 空格分隔
        ans = -1        # 赋值为-1，没有找到直接返回
        for i in range(len(part)):      # 遍历，查看是否为某个单词前缀
            if part[i].startswith(searchWord):      # 如果是某一单词前缀
                ans = i + 1             # index从1开始
                break 
        return ans
...
if __name__ == "__main__":
    ...
    searchWord = "burg"
    sol = Solution()
    result = sol.isPrefixOfWord(sentence, searchWord)
    print(result)
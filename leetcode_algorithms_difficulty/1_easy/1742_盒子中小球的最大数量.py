...
class Solution:
    def countBalls(self, lowLimit: int, highLimit: int) -> int:
        nums = defaultdict(int)                     # 默认每个value为 0
        for ball in range(lowLimit, highLimit + 1): # 遍历球的编号，
            ball_num = sum(map(int,str(ball)))      # 计算每位数字的和
            nums[ball_num] += 1                     # 字典中数字和对应的 value 加一
...
if __name__ == "__main__":
    ...
    highLimit = 10
    sol = Solution()
    result = sol.countBalls(lowLimit,highLimit)
    print(result)